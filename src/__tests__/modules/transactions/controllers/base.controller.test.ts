...
describe('Transactions BaseController', () => {
  ...
  it('sendSuccess calls ResponseHandler.success and returns value', () => {
    ...
    const result = controller.testSendSuccess(mockRes, { message: 'ok', data: { foo: 1 }, statusCode: 201 });
  ...
  it('sendError calls ResponseHandler.error and returns value', () => {
    ...
    const result = controller.testSendError(mockRes, { message: 'fail', statusCode: 400, error: { code: 'X', details: { foo: 2 } } });
    expect(spy).toHaveBeenCalledWith(mockRes, { message: 'fail', statusCode: 400, error: { code: 'X', details: { foo: 2 } } });
